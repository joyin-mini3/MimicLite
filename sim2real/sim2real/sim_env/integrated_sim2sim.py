...
G1_TERMINATION_ROOT_BODY_NAME = "torso_link"
...
class IntegratedPolicyRuntime:
    def __init__(
        ...
        *,
        ...
        if self.robot_cfg.strict_joint_contract:
            self._validate_strict_policy_config()
    ...
    def _validate_strict_policy_config(self) -> None:
        expected = self.action_joint_names
        if self.joint_names_simulation != expected:
            raise ValueError(
                "Strict policy joint_names_simulation mismatch; "
                f"expected={expected}, actual={self.joint_names_simulation}"
            )
        policy_joint_names = list(self.policy_config.get("policy_joint_names", []))
        if policy_joint_names != expected:
            raise ValueError(
                "Strict policy action joint order mismatch; "
                f"expected={expected}, actual={policy_joint_names}"
            )
        for key in ("default_joint_pos", "joint_kp", "joint_kd"):
            values = self.policy_config.get(key)
            if not isinstance(values, dict) or list(values) != expected:
                actual = list(values) if isinstance(values, dict) else values
                raise ValueError(
                    f"Strict policy {key} must cover every joint exactly once in canonical "
                    f"order; expected={expected}, actual={actual}"
                )
        action_scale = self.policy_config.get("action_scale")
        if not isinstance(action_scale, list) or len(action_scale) != len(expected):
            raise ValueError(
                "Strict policy action_scale must be a canonical per-joint list; "
                f"expected_length={len(expected)}, actual={action_scale}"
            )
        evaluation = self.policy_config.get("evaluation")
        required_evaluation_keys = (
            "tracking_body_names",
            "termination_root_body_name",
            "anchor_body_name",
        )
        if not isinstance(evaluation, dict):
            raise ValueError("Strict policy requires an evaluation mapping")
        missing = [key for key in required_evaluation_keys if key not in evaluation]
        if missing:
            raise ValueError(f"Strict policy evaluation mapping is missing {missing}")
        tracking_body_names = [str(name) for name in evaluation["tracking_body_names"]]
        if not tracking_body_names or len(tracking_body_names) != len(set(tracking_body_names)):
            raise ValueError(
                "Strict policy evaluation.tracking_body_names must be non-empty and unique"
            )
        for body_name in (
            *tracking_body_names,
            str(evaluation["termination_root_body_name"]),
            str(evaluation["anchor_body_name"]),
        ):
            if body_name not in self.body_names_simulation:
                raise ValueError(
                    f"Strict policy evaluation body {body_name!r} is not in body_names_simulation"
                )
        motion = self.policy_config.get("motion")
        if not isinstance(motion, dict):
            raise ValueError("Strict policy requires a motion mapping")
        if list(motion.get("joint_names", [])) != expected:
            raise ValueError(
                "Strict policy motion.joint_names mismatch; "
                f"expected={expected}, actual={motion.get('joint_names')}"
            )
        if str(motion.get("anchor_body_name")) != str(evaluation["anchor_body_name"]):
            raise ValueError(
                "Strict policy motion/evaluation anchor_body_name values must match"
            )
        if str(motion.get("root_body_name")) not in self.body_names_simulation:
            raise ValueError(
                f"Strict policy motion root body {motion.get('root_body_name')!r} is invalid"
            )
    ...
    def _resolve_joint_array(self, key: str) -> np.ndarray:
        ...
        if self.robot_cfg.strict_joint_contract:
            return np.asarray(
                [values_dict[name] for name in self.action_joint_names],
                dtype=np.float32,
            )
...
class IntegratedSimRuntime:
    ...
    def _init_mappings(self) -> None:
        ...
        for actuator_id in range(self.mj_model.nu):
            if self.mj_model.actuator_trntype[actuator_id] != mujoco.mjtTrn.mjTRN_JOINT:
                continue
            target_joint_id = int(self.mj_model.actuator_trnid[actuator_id, 0])
            target_joint_name = self.mj_model.joint(target_joint_id).name
        ...
        for name in self.robot_cfg.joint_names:
            if name not in joint_names_mujoco:
                raise ValueError(f"Controlled joint {name!r} is missing from the MuJoCo model")
            target_actuator_ids = actuator_ids_by_target.get(name, [])
            if len(target_actuator_ids) != 1:
                raise ValueError(
                    f"Controlled joint {name!r} must map to exactly one actuator by "
                    f"transmission target, got {target_actuator_ids}"
                )
            ...
            self.act_adrs.append(target_actuator_ids[0])

        if self.robot_cfg.strict_joint_contract:
            scalar_joint_names = [
                self.mj_model.joint(joint_id).name
            ...
            if scalar_joint_names != list(self.robot_cfg.joint_names):
                raise ValueError(
                    f"Robot '{self.robot_cfg.name}' strict MuJoCo joint order mismatch; "
                    f"expected={list(self.robot_cfg.joint_names)}, actual={scalar_joint_names}"
                )
            if self.mj_model.nu != len(self.robot_cfg.joint_names):
                raise ValueError(
                    f"Robot '{self.robot_cfg.name}' strict model must have "
                    f"nu={len(self.robot_cfg.joint_names)}, got {self.mj_model.nu}"
                )
        ...
        self.joint_effort_limit_mjc = np.asarray(
            [self.robot_cfg.joint_effort_limit[name] for name in self.robot_cfg.joint_names],
            dtype=np.float32,
        )
        self.joint_idx_in_ctrl = np.asarray(self.act_adrs, dtype=int)
...
class IntegratedSim2Sim:
    def __init__(self, args: "IntegratedSim2SimArgs"):
        ...
        evaluation_cfg = self.policy.policy_config.get("evaluation")
        if isinstance(evaluation_cfg, dict):
            self.tracking_body_patterns = tuple(
                str(name) for name in evaluation_cfg["tracking_body_names"]
            )
            self.termination_root_body_name = str(
                evaluation_cfg["termination_root_body_name"]
            )
            self.anchor_body_name = str(evaluation_cfg["anchor_body_name"])
        elif self.robot_cfg.strict_joint_contract:
            raise ValueError(
                f"Robot '{self.robot_cfg.name}' may not use G1 evaluation fallbacks"
            )
        else:
            self.tracking_body_patterns = G1_TRACKING_BODY_PATTERNS
            self.termination_root_body_name = G1_TERMINATION_ROOT_BODY_NAME
            self.anchor_body_name = G1_ANCHOR_BODY_NAME
    ...
    def _update_tracking_failure_state(
        ...
        tracking_indices = self._indices_for_patterns(names, self.tracking_body_patterns)
        root_idx = names.index(self.termination_root_body_name)
        anchor_idx = names.index(self.anchor_body_name)
        ...
        self.trajectory.append(
            {
                **root_frame,
                "robot_joint_vel": np.asarray(
                    self.sim.mj_data.qvel[self.sim.qvel_adrs],
                    dtype=np.float32,
                ).copy(),
    ...
    def _save_trajectory(self) -> None:
        ...
        joint_velocity_limit = np.asarray(
            [
                self.robot_cfg.joint_velocity_limit[name]
                for name in self.robot_cfg.joint_names
            ],
            dtype=np.float32,
        )
        abs_joint_vel = np.abs(robot_joint_vel)
        joint_velocity_max_abs = abs_joint_vel.max(axis=0)
        joint_velocity_p99_abs = np.percentile(abs_joint_vel, 99.0, axis=0).astype(
            np.float32
        )
        joint_velocity_exceed_fraction = (
            abs_joint_vel > joint_velocity_limit.reshape(1, -1)
        ).mean(axis=0, dtype=np.float64).astype(np.float32)

        np.savez_compressed(
            output_path,
            joint_names=np.asarray(self.robot_cfg.joint_names),
            robot_joint_vel=robot_joint_vel,
            joint_velocity_limit=joint_velocity_limit,
            joint_velocity_max_abs=joint_velocity_max_abs,
            joint_velocity_p99_abs=joint_velocity_p99_abs,
            joint_velocity_exceed_fraction=joint_velocity_exceed_fraction,
            robot_root_pos_w=np.stack(
        ...
        logger.info(
            "Joint velocity report (name: max/p99/limit/exceed): {}",
            ", ".join(
                f"{name}: {max_abs:.3f}/{p99_abs:.3f}/{limit:.3f}/{exceed:.2%}"
                for name, max_abs, p99_abs, limit, exceed in zip(
                    self.robot_cfg.joint_names,
                    joint_velocity_max_abs,
                    joint_velocity_p99_abs,
