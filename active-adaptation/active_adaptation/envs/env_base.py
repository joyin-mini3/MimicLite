...
class _EnvBase(EnvBase, RegistryMixin):
    ...
    def _setup_simulation(self):
        ...
        sim_steps = self.step_dt / self.physics_dt
        self.decimation = int(round(sim_steps))
        if self.decimation <= 0 or not np.isclose(
            sim_steps,
            self.decimation,
            rtol=1e-6,
            atol=1e-9,
            ...
            raise ValueError(
                "sim.step_dt must be a positive integer multiple of the physics dt, "
                f"got step_dt={self.step_dt}, physics_dt={self.physics_dt}, ratio={sim_steps}"
            )
