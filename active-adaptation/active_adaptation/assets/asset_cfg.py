...
def _resolve_asset_path(value: str | Path) -> Path:
    value_str = str(value)
    if value_str.startswith("hf://"):
        from mjhub import resolve_asset_reference

        return Path(resolve_asset_reference(value_str))
...
class AssetCfg:
    ...
    def isaaclab(self):
        ...
        asset_path = _resolve_asset_path(self.usd_path)
        ...
        return ArticulationCfg(
            ...
            strict_joint_contract=self.strict_joint_contract,
        )
    ...
    def mujoco(self):
        ...
        return MJArticulationCfg(
            mjcf_path=str(_resolve_asset_path(self.mjcf_path)),
            init_state={
